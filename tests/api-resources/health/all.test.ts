...
describe('resource all', () => {
  test('retrieveAll', async () => {
    const responsePromise = client.health.all.retrieveAll();
  ...
  test('retrieveAll: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.health.all.retrieveAll({ path: '/_stainless_unknown_path' })).rejects.toThrow(
      Ngc.NotFoundError,
    );
  });
  ...
  test('retrieveAll: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.health.all.retrieveAll(
        { secret: 'secret', showDetails: true },
