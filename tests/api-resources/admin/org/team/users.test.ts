...
describe('resource users', () => {
  test('removeRole', async () => {
    const responsePromise = client.admin.org.team.users.removeRole('org-name', 'team-name', 'id');
  ...
  test('removeRole: request options instead of params are passed correctly', async () => {
    ...
    await expect(
      client.admin.org.team.users.removeRole('org-name', 'team-name', 'id', {
  ...
  test('removeRole: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.admin.org.team.users.removeRole(
        'org-name',
        'team-name',
        'id',
